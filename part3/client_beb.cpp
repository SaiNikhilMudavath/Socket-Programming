...
void* handle_clients(void* arg)
{
    ...
    if ((sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        std::cerr << "Socket creation failed for client: "<<client_id<<endl;
    ...
    if (inet_pton(AF_INET, ip.c_str(), &serv_addr.sin_addr) <= 0) {
        std::cerr << "Invalid address or address not supported for client"<<client_id<<endl;
        pthread_exit(nullptr);
    }
    ...
    while(!eof)
    {
        ...
        // cout<<"for client "<<client_id<<" attempt is "<<attempt<<" and wait time is "<<wait_time<<"and offset is "<<offset<<endl;
