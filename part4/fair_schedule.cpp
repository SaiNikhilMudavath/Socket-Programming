...
vector<queue<client_request*>> request_queue;
...
client_request* queue_fun(string func,int i,client_request* c)
{
    lock_guard<mutex> lock(mtx);
    if(func=="push")
    {
        request_queue[i].push(c);
        // cout<<"pushed into the queue successfully"<<endl;
    }
    else if(func=="front")
    {
        if(request_queue[i].empty()) return NULL;
        // cout<<"checking for front  :"<<request_queue[i].size()<<endl;
        client_request* y=request_queue[i].front();
        request_queue[i].pop();
        return y;
    }
    return NULL;
...
void* service(void* arg)
{
    ...
    while (true)
    {
        for(int i=1;i<=original_clients;i++)
        {
            // cout<<"entered while"<<endl;
            client_request* requests = queue_fun("front",i, NULL);
            if (requests != NULL)
            {
    ...
    while (true)
    {
        ...
        for(int index=0;index<off.size();index++)
        {
            if(off[index]=='\n')
            {
                ...
                queue_fun("push",map_queue[new_socket],requests);
                start=index+1;
            }
        }
    }
    // cout<<"closing client successfully"<<endl;
    close(new_socket);
    pthread_exit(NULL);
}
...
int main()
{
    ...
    request_queue.resize(original_clients+1);
