...
client_request* queue_fun(string func,client_request* c)
{
    lock_guard<mutex> lock(mtx);
    if(func=="push")
    {
    ...
    else if(func=="front")
    {
        ...
        if(request_queue.empty()) return NULL;
...
void* service(void* arg)
{
    ...
    while (true && higher<original_clients)
    {
        // cout<<"entered while"<<endl;
        client_request* requests = queue_fun("front", NULL);
        if (requests != NULL)
        {
